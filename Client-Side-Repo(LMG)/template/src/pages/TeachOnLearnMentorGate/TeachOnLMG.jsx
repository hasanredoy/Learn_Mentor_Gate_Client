...
const TeachOnLMG = () => {
  ...
  const {data:isTeacherPending,isLoading,refetch}=useQuery({
    
    // enabled: !userEmail,
    retry:userEmail,
    queryKey:['isTeacherPending'],
    queryFn:async()=>{
        const {data}=await axiosSecure.get(`/teacher-status?email=${userEmail}`)
        return data
    }
  })
  if(userEmail){
 ...
 const mutateAsync=usePostTeacher()
  const handlePost = async (e) => {
    ...
    const teacherData = { name:user?.displayName, photo:user?.photoURL, email:user?.email , title,category,experience,status:'pending'};
  ...
  return (
    <div className="  max-w-7xl mx-auto mt-20">
      <h1 className=" text-2xl lg:text-4xl py-4 font-bold text-center">Hi Welcome  {user?.displayName? <span className=" text-amber-600">{user?.displayName}</span>:"Back"},Teach on <span className=" text-green-600">Learn Mentor Gate?</span></h1>
      ...
      <div className="card shrink-0 shadow-2xl  w-full h-full  lg:w-2/3 mx-auto my-10 bg-base-200">
        <h1 className=" text-xl font-bold text-center py-5">Please Fill The From Given Blew !</h1>
        <form  onSubmit={handleSubmit(handlePost)} className="card-body">
          ...
          <div className="form-control mt-6">
            <button
            type="submit" className={`text-lg font-bold text-white bg-green-700 px-4 py-2 rounded-lg hover:bg-white hover:text-green-600 hover:border hover:border-green-700 ${isTeacherPending.status==='pending'?'btn-disabled bg-gray-300':'btn'} `}>
            {isTeacherPending?.status?isTeacherPending?.status==='pending'?'Submit for review':' Request to another':'Submit For Review'}
            </button>
          </div>
        </form>
